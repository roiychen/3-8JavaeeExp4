...
public class ProductController {
    ...
    @GetMapping("")
    public ReturnObject searchProductByName(@RequestParam String name, @RequestParam(required = false, defaultValue = "manual") String type) {
        ReturnObject retObj = null;
        List<Product> productList = null;
        if (null != type && "manual".equals(type)){
            productList = productService.findProductByName_manual(name);
        }
        else if(null!=type && "new".equals(type)) {
            logger.info("searchProductByName: Invoking NewProductByName with name: {}", name);
            productList = productService.JoinProductByName(name);
        }
        else {
            logger.info("searchProductByName: Invoking retrieveProductByName with name: {}", name);
